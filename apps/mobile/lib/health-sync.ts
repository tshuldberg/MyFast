...
function getWeightUnit(db: Database): WeightUnit {
  const value = getSetting(db, 'weightUnit');
  return value === 'kg' ? 'kg' : 'lbs';
}

function getDefaultImportStartIso(db: Database): string {
...
function getExportCursor(db: Database): string | null {
  return getSetting(db, EXPORT_CURSOR_KEY);
}

function listCompletedFastsForExport(db: Database): CompletedFastRow[] {
  const since = getExportCursor(db);
  if (since) {
    return db.all<CompletedFastRow>(
...
function insertImportedWeightIfMissing(
  ...
  const exists = db.get<{ count: number }>(
  ...
  db.run(
    `INSERT INTO weight_entries (id, weight_value, unit, date, notes, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      createId('weight'),
      rounded,
  ...
  if (options.writeFasts) {
    ...
    if (fasts.length > 0) {
      const payload = fasts.map((fast) => ({
        ...
        notes: `Fasting window tracked in MyFast (${fast.target_hours}h target).`,
        metadata: {
          clientRecordId: `myfast-fast-${fast.id}`,
          ...
          appleHealthKit.saveWorkout(
            {
              ...
              metadata: {
                sourceName: 'MyFast',
                externalUUID: `myfast-fast-${fast.id}`,
              },
            },
            cb,
          ),
...
/**
 * Synchronous baseline status for UI hydration.
 */
...
/**
 * Runtime probe that verifies native health modules are actually available.
 */
