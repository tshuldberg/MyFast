...
import type { SummaryStats } from '../types/index';
...
function summaryForRange(db: Database, start: string, end: string): SummaryStats {
  const totals = db.get<{
    total_fasts: number;
  ...
  // currentStreak remains global streak to keep UI consistent with existing app behavior.
...
export function getMonthlySummary(db: Database, year: number, month: number): SummaryStats {
  const range = monthRange(year, month);
  return summaryForRange(db, range.start, range.end);
}

export function getAnnualSummary(db: Database, year: number): SummaryStats {
